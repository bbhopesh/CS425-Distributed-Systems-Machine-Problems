...
// Code borrowed from http://www.java2s.com/Tutorial/Java/0140__Collections/CyclicIteration.htm
/**
...
public class CyclicIteration<K, V> implements Iterable<Map.Entry<K, V>> {
  ...
  private final boolean startKeyExclusive;

  /** Construct an {@link Iterable} object,
   * so that an {@link Iterator} can be created  
   * for iterating the given {@link SortedMap}.
   * The iteration begins from the starting key exclusively if flag is true else inclusively.
   */
  public CyclicIteration(SortedMap<K, V> navigablemap, K startingkey, boolean startKeyExclusive) {
  ...
  private class CyclicIterator implements Iterator<Map.Entry<K, V>> {
    ...
    private CyclicIterator() {
      ...
      if (hasnext) {
        ...
        if (startKeyExclusive && e.getKey().equals(startingkey)) {
          e = nextEntry();
        }
